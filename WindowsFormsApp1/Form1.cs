...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        SpeechSynthesizer Maahi = new SpeechSynthesizer();
        ...
        private void Default_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            if (speech == "Hello")
            {
                Maahi.SpeakAsync("Hello, I am here");
            }

            if (speech == "How are you")
            {
                Maahi.SpeakAsync("I am working normally");
            }

            if (speech == "What time is it")
            {
                Maahi.SpeakAsync(DateTime.Now.ToString("h mm tt"));
            }

            if (speech == "Stop talking")
            {
                Maahi.SpeakAsyncCancelAll();
                ranNum = rnd.Next(1, 2);
                if (ranNum == 1)
                {
                    Maahi.SpeakAsync("Yes sir");
                }
                if (ranNum == 2)     
                {
                    Maahi.SpeakAsync("I am sorry i will be quiet");
                }
            }
            if (speech == "Stop listening")
            {
                Maahi.SpeakAsync("if you need me just ask");
        ...
        private void startlistening_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            if (speech == "Wake up")
            {
                ...
                Maahi.SpeakAsync("Yes, I am here");
                _recognizer.RecognizeAsync(RecognizeMode.Multiple); 
